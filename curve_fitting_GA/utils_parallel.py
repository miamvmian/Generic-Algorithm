...
from model import eval_func, BOUNDS, PHYS_BOUNDS
from utils import decode_chromosome, param_constraints, int2phys, setup_logger
    ...
    def run(self):
        ...
        while not self.terminate_flag.value:  # 检查共享的终止标志
            ...
            try:
                ...
                # two steps to decode chromosome into real physic parameters
                int_params = decode_chromosome(ind)
                phys_params = int2phys(int_params, BOUNDS, PHYS_BOUNDS)

                # To check if the params meets the constraint requirement
                start_time = time.time()
                if not param_constraints(int_params):
...
class ParallelGeneticAlgorithm:
    def __init__(self, eval_func, individual_generator, crossover_op, mutation_op, selection_op, target=None,
        ...
        self.toolbox.register("individual", tools.initIterate, creator.Individual,
                              lambda: self.individual_generator(BOUNDS))
    ...
    def _get_fitness(self, hashable_ind, penalty=10):
        ...
        # fitness: relative root mean square error.
            ...
            fitness = self.eval_fitness(self._TARGET, result)
            return (fitness,)
    ...
    def run(self, generations=50, clear_cache=False, stats=None, verbose=False):
        ...
        try:
            ...
            for gen in range(generations):
                ...
                # k-best selection + tournament selection
                pop = tools.selBest(offspring, k=10) + self.toolbox.select(offspring, len(offspring)-10, tournsize=3)
